package com.proxiad.task.ivanboyukliev.userscrudapp.utils;

public final class ApplicationConstants {

  // APP End-points
  public static final String GET_ALL_USRS_URL = "/getAllUsrs";
  public static final String DELETE_USR_URL = "/delete";
  public static final String INSERT_USR_URL = "/insertUser";
  public static final String UPDATE_USR_REQ = "/updateRequest";
  public static final String UPDATE_USR_URL = "/updateUser";


}
